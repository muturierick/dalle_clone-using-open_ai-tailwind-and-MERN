...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://muturi-erick-dalle-clone.onrender.com/api/v1/dalle", {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(form.prompt && form.photo){
      ...
      try {
        const response = await fetch('https://muturi-erick-dalle-clone.onrender.com/api/v1/post', {
          method: "POST",
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-5 flex gap-5 ">
          <button
            ...
            className="text-white bg-green-700 rounded-md text-sm font-medium
          rounded-md text-sm w-full dm:w-auto px-5 py-2.5 text-center"
          >
